...
function checkKeySet(keys) {
  ...
  assert.equal(
    typeof keys.privateKeys.owner,
    "string",
    "keys.privateKeys.owner"
  );
  assert.equal(
    typeof keys.privateKeys.active,
    "string",
    "keys.privateKeys.active"
  );
  assert.equal(
    typeof keys.privateKeys.posting,
    "string",
    "keys.privateKeys.posting"
  );
  assert.equal(typeof keys.privateKeys.memo, "string", "keys.privateKeys.memo");
  ...
  assert.equal(
    typeof keys.publicKeys.active,
    "string",
    "keys.publicKeys.active"
  );
  assert.equal(
    typeof keys.publicKeys.posting,
    "string",
    "keys.publicKeys.posting"
  );
  assert.equal(typeof keys.publicKeys.memo, "string", "keys.publicKeys.memo");
}
